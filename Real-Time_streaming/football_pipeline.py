...
def create_driver():
    ...
    # Notice headless=False here! The virtual display handles the "headless" part.
...
def scrape_whoscored(url: str, driver) -> Optional[dict]:
    try:
        try:
            ...
            pass  # expected with page_load_strategy='none'

        # Poll every 3 seconds until matchCentreData appears (max 60s)
        log.info("Waiting for matchCentreData to load...")
        for attempt in range(20):
            time.sleep(3)
            page = driver.page_source
            if "matchCentreData" in page:
                log.info(f"matchCentreData found after {(attempt+1)*3}s")
                break
            log.info(f"Attempt {attempt+1}/20 — not ready yet...")
        else:
            log.error("matchCentreData never appeared after 60s")
        ...
        if not element:
            log.error("Found keyword but could not select script element")
            return None
...
def send_event(producer: Producer, event: dict, match_url: str):
    ...
    event['event_timestamp'] = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')
    ...
    # ── Force ID fields to clean integers ──────────────────
    for key_col in ['id', 'event_id', 'team_id', 'player_id']:
        if clean.get(key_col) is not None:
            try:
                clean[key_col] = int(float(clean[key_col]))
            except:
                clean[key_col] = 0
    # ────────────────────────────────────────────────────────
    ...
    try:
        ...
        try:
            ...
            conn = s2.connect(
                ...
                port     = 3333,
                user     = 'ahmed-580bb',
                password = '6b3OA{HT6_{m2XxW?6w,C',
                database = 'football_live'
            )
            conn.cursor().execute("TRUNCATE TABLE match_events_live")
            conn.commit()
            conn.close()
            log.info("🗑️  Cleared previous match data")
        except Exception as e:
            log.warning(f"Could not clear old data: {e}")
        # ────────────────────────────────────────────────────────────────
...
<html>
<head>
  <title>Football Intelligence Hub Control Panel</title>
  <style>
...
<body>
<div class="box">
  <h1>⚽ Football Intelligence Hub</h1>
...
if __name__ == '__main__':
    log.info("🚀 Football Match Streaming Pipeline starting...")
    log.info(f"   Kafka broker : {KAFKA_BROKER}")
    log.info(f"   Replay speed : {REPLAY_SPEED}x")
    log.info("   Open browser : http://44.192.89.78:5000")
